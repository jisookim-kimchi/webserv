...
class ServerConfig {
   public:
    ...
    const std::map<uint16_t, std::string>& getErrorPagePath() const {
        return error_page_path_;
    }
    ...
    void addErrorPagePath(const uint16_t& error_code, const std::string& file_path) {
        error_page_path_[error_code] = file_path;
    }
    void addServerName(const std::string& server_name) {
        server_name_.push_back(server_name);
    }
    void addPort(const uint16_t port) {
